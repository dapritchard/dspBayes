...
int XGen::calc_prior_probs(double* prior_probs, XMissCyc* xmiss_cyc, XMissDay* xmiss_day) {
...
void XGen::calc_posterior_probs(double* posterior_probs,
    ...
    // and
    //
    //     P(X_{ijk_r} = x_{ijk_r} | X_ij(k_r-1))
    //
	...
	// value from the day before the fertile window differs than for the
	// remaining cases.
	...
	// case: x_{ijk_1} = 0 for the t-th permutation
	else {
	    prod_priors = 1 - prior_probs[0][day_before_fw_sex];
	    curr_x = 0;
	}

	// perform the calculations for the remaining missing elements
	for (int r = 1; r < curr_miss_cyc->n_miss; ++r) {
	    ...
	    // case: x_{ijk_r} = 0 for the current permutation
	    else {
		prod_priors *= (curr_miss_day[r].prev != SEX_MISS) ?
	...
	// case: at least one day with intercourse occurred during the cycle
	// (i.e. we have a random variable)
	if (sum_exp_ubeta > 0) {
	    sum_exp_ubeta *= *(xi.vals() + curr_miss_cyc.subj_idx);
	    probs[t] = (1 - exp(-sum_exp_ubeta)) * prod_priors;
	}
...
int XGen::sample_x_perm(double* probs, int n_perms) {
