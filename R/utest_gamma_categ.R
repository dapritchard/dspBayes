utest_gamma_categ <- function(dsp_data, W, xi, ubeta, seed_val) {
    ...
    calc_a_tilde <- function(hyp_a, W) {
        hyp_a + sum(W)
    }

    calc_b_tilde <- function(hyp_b, xi_day, ubeta_no_h) {
        hyp_b + sum(exp(log(xi_day) + ubeta_no_h))
    }
    ...
    calc_xi_day <- function(xi, subj_day_blocks) {
        ...
        rep(seq_along(subj_day_blocks), n_subj_days)
    }
    ...
    # calculate `a_tilde`
    a_tilde <- calc_a_tilde(hyp_a, W)

    # calculate `b_tilde`
    ubeta_no_h <- ubeta - (dsp_data$U[, h] * beta_prev)
    b_tilde <- calc_b_tilde(hyp_b, xi_day, ubeta_no_h)
    ...
    gamma_specs_zero_half <- replace(gamma_specs_all, c("hyp_p", "bnd_u"), c(0, 1))
    specs <- list(gamma_specs_all       = gamma_specs_all,
