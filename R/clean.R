remove_cyc_with_miss <- function(comb_dat, var_nm, fw_incl, use_na) {
    ...
    # case: use missing for covariates, but not for intercourse
        ...
        check_idx <- which(colnames(comb_dat) == c(var_nm$sex, var_nm$preg))
    }
    # case: don't use any cycles with missing
    else if (identical(use_na, "none")) {
        check_idx <- seq_along(comb_dat)
    }
    # case: use all missing (except that we still need pregnancy data)
    else {
        check_idx <- which(colnames(comb_dat) == var_nm$preg)
    }
    ...
    # the coumns indexed by `check_idx`, and if none are missing then stores the
    # indices corresponding to the cycle in `keep_idx_list`
...
map_sex_to_bool <- function(sex_vec) {
