strip_days <- function(comb_dat, var_nm, fw_incl, use_na) {
    ...
    if (! ((use_na == "covs") || (use_na == "all"))) {
        covs_miss_bool <- vector("logical", nrow(comb_dat))
    }
    else {
        # whether any covariates are missing
        covs_miss_bool <- !complete.cases(comb_dat[, var_nm$covs])
    }

    # whether intercourse occured on the given day
    sex_yes_bool <- map_vec_to_bool(comb_dat[[var_nm$sex]])
    sex_yes_bool[is.na(sex_yes_bool)] <- FALSE
