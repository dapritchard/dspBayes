...
#'     term in the \code{formula} parameter.
#'
...
#'     specified by the left-hand term in the \code{formula} parameter.
#'
...
#'     the \code{formula} parameter, then it is interpreted to mean that a
...
dspDat <- function(formula,
    ...
    # varNames <- getVarNames(formula, baseline, cycle, daily, id_name, cyc_name, sex_name, fw_name)
    ...
    # combDat <- getCombDat(formula, redDat, varNames, fw_len, cycList)

    # # Create X, Y, and U (from the Dunson and Stanford paper)
    # modelObj <- getModelObj(combDat, formula, varNames, fw_len)

    var_nm <- consolidate_var_nm(formula, baseline, cycle, daily, id_name,
                                 cyc_name, sex_name, fw_name)

    comb_dat <- merge_dsp_dat(daily, cycle, baseline, vars_nm, min_days_req)
    ...
    # samplerObj <- getSamplerObj(modelObj, redDat$day[[cyc_name]], fw_len, use_na)
    # samplerObj <- c(samplerObj, formula=formula, use_na=use_na)
