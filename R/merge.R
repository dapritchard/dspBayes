merge_dsp_data <- function(daily, cycle, baseline, vars_nm, min_days_req) {

    # # the union of all of the variables that we need across the various datasets
    # vars_nm <- c(id_name, cyc_name, sex_name, fw_name, all.vars(formula))

    # reduce each dataset to only contain the variables that we need.  Returns
    # NULL if the input is NULL.
    base_red <- get_red_dat(baseline, vars_nm$all)
    cyc_red <- get_red_dat(cycle, vars_nm$all)
    day_red <- get_red_dat(daily, vars_nm$all)

    # obtain every unique subject id and cycle pair for any cycle that has
    # pregnancy outcome data
    keypairs <- get_keypairs(day_red, cyc_red, vars_nm)
    ...
    keep_idx <- day_red[[vars_nm$fw]] %in% fw_incl %>% which
    ...
    day_formatted <- construct_day_obs(day_red, keypairs, fw_incl, vars_nm, min_days_req)
...
get_var_nm <- function(formula, baseline, cycle, daily, id_name, cyc_name, sex_name, fw_name) {
    ...
    # the union of all of the variables that we need across the various datasets
    model_vars_nm$all <- c(id_name, cyc_name, sex_name, fw_name, all.vars(formula))

    # store name info for convenience
    vars_nm$id  <- id_name
    vars_nm$cyc <- cyc_name
    vars_nm$sex <- sex_name
    vars_nm$fw  <- fw_name
...
get_red_dat <- function(dataset, all_vars_nm) {
    ...
    # `all_vars_nm`
    var_incl_bool <- colnames(dataset) %in% all_vars_nm
...
#      of these columns is given by `vars_nm`.

get_keypairs <- function(daily, cycle, vars_nm) {
    ...
    if ((vars_nm$preg %in% colnames(daily)) && (vars_nm$preg %in% colnames(cycle))) {
        keypairs_df <- rbind(cycle[, c(vars_nm$id, vars_nm$cyc)],
                             daily[, c(vars_nm$id, vars_nm$cyc)],
                             stringsAsFactors = FALSE)
    }
    # case: only the daily data constains pregnancy information
    else if (vars_nm$preg %in% colnames(daily)) {
        keypairs_df <- daily[, c(vars_nm$id, vars_nm$cyc)]
    }
    # case: only the cycle data constains pregnancy information
    else if (vars_nm$preg %in% colnames(cycle)) {
        keypairs_df <- cycle[, c(vars_nm$id, vars_nm$cyc)]
    }
    # case: can't find the pregnancy status variable
    else {
        paste0("cannot find ", vars_nm$preg, " in cycle or daily") %>% stop(call. = FALSE)
    }
...
#      atomic vector. `vars_nm` is a list providing the names of the id, cycle,
#      and fertile window columns.  `min_days_req` is a length-1 numeric vector.

construct_day_obs <- function(daily, keypairs, fw_incl, vars_nm, min_days_req) {
    ...
    for (k in 1:n_cycle) {
        ...
        curr_id <- keypairs[k, vars_nm$id]
        curr_cyc <- keypairs[k, vars_nm$cyc]
        ...
        # appropriate length.
        curr_df <- template_df
        curr_df[[vars_nm$id]] <- curr_id
        curr_df[[vars_nm$cyc]] <- curr_cyc
        curr_df[[vars_nm$fw]] <- fw_incl

        # reduce to daily to the current subject and cycle
        curr_day_idx <- (daily[[vars_nm$id]] == curr_id &
                         daily[[vars_nm$cyc]] == curr_cyc) %>% which
        ...
        for (i in seq_along(fw_incl)) {
            ...
            row_idx <- which(curr_daily[[vars_nm$fw]] == fw_incl[i])
